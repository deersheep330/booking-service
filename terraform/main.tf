terraform {
  backend "s3" {
    ...
    key = "booking-service/terraform.tfstate"
    region = "us-east-2"
  }
}
...
data "template_file" "container_definition" {
  ...
  vars = {
    image = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.region.name}.amazonaws.com/booking:${var.IMAGE_VERSION}"
    vaccine_line_token = var.VACCINE_LINE_TOKEN
